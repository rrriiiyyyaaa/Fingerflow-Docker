...
cut_off = sys.argv[1] if len(sys.argv)>1 else 5
cut_off = int(cut_off) if int(cut_off) in [0,1,2,3,4,5] else 5
print("Cut off:", cut_off)
overlay_ = overlay_ / f"cutoff_{cut_off}"
overlay_.mkdir(exist_ok=True)
for p in base.glob("*.png"):
    ...
    # mv = [m for m in mv if m['type']<=1]  # keep only ridge ending and bifurcation
    # with open(str(out_), 'w') as f:
    #     json.dump(mv, f, indent=4)
    with open(str(out_), 'r') as f:
        mv = json.load(f)
    
    mv = [m for m in mv if int(m['class'])<=cut_off]  # keep only ridge ending and bifurcation
